#exercise7

# Loop 1: Write a loop that counts up from 0 to 50 in increments of 1. 
print("Counting up from 0 to 50 in increments of 1:")
for i in range(0, 51, 1):
    print(i)

# Loop 2:write a loop that Counts down from 50 to 0 in decrements of 1
print("\nCounting down from 50 to 0 in decrements of 1:")
for i in range(50, -1, -1):
    print(i)

# Loop 3: write a loop that  Counts up from 30 to 50 in increments of 1
print("\nCounting up from 30 to 50 in increments of 1:")
for i in range(30, 51, 1):
    print(i)

# Loop 4:write a loop that  Count down from 50 to 10 in decrements of 2
print("\nCounting down from 50 to 10 in decrements of 2:")
for i in range(50, 9, -2):
    print(i)

# Loop 5:write a loop that  Count up from 100 to 200 in increments of 5
print("\nCounting up from 100 to 200 in increments of 5:")
for i in range(100, 201, 5):
    print(i)
